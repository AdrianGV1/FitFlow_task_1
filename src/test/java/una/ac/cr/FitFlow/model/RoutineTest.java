...
class RoutineTest {
    ...
    void builderAndAccessors_ok() {
        ...
        Routine r = Routine.builder()
                ...
                // no pasamos activities/progressLogs: deben inicializarse vacíos por
                // @Builder.Default
                .build();
    ...
    void daysOfWeekMapping_ok() throws Exception {
        ...
        // Tipo de campo: debe ser List (no podemos verificar genéricos en runtime, pero
        // al menos el nombre)
        assertThat(List.class.isAssignableFrom(f.getType())).isTrue();
        assertThat(f.getGenericType().getTypeName()).contains("Routine.DaysOfWeek");

        // Verifica el @Column usado en tu entidad
        Column col = f.getAnnotation(Column.class);
        assertThat(col).isNotNull();
        assertThat(col.name()).isEqualTo("days_of_week");
        assertThat(col.nullable()).isFalse();

        // Asegura que NO estamos usando @ElementCollection/@CollectionTable en tu
        // diseño actual
